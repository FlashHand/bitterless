...
Ral 定了「以 cowork 为准就是蓝点」。**颜色取 bl chat 自己的强调色 `#4e5882`**
（`ChatPanel.less` 里发送按钮用的那个，Royal Blue），不是 cowork 的 `#165dff`：
两者都是蓝，但 bl 里已经有一个强调蓝了，再引入第二个会在同一个面板里打架。
**这一处是有意偏离 cowork 的字面色值、保留它的设计意图**（蓝 = 等你看，灰 = 还没到你）。
