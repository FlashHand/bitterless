...
assert(handler.includes("window.on('close'") && handler.includes('window.hide()'), 'normal Maestro close must hide and preserve its live runtime')
assert(handler.includes("window.once('closed'") && handler.includes('destroyMaestroRuntime()'), 'explicit Maestro destruction must clean its full runtime')
