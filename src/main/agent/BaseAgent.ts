...
export const prependAgentPreamble = (text: string, preamble: string): string =>
...
export interface BaseAgentOptions {
  ...
  modelsPath?: string
  /** pi agent dir. Maestro passes `<userData>/.pi` (maestroAgentDir) so pi's global AGENTS.md /
   * SYSTEM.md / skills / settings come from the app, not the user's own `pi` CLI. */
  agentDir?: string
...
export class BaseAgent {
  ...
  async readContext(): Promise<{ runtime: AgentRuntimeContextSnapshot | null; preamblePending: boolean }> {
    if (this.disposed) throw new Error('Agent has been disposed.')
    const existing = this.sessionPromise
    if (!existing) return { runtime: null, preamblePending: true }
    const session = await existing
    if (existing !== this.sessionPromise || this.disposed) throw new Error('The model session changed during context export. Retry.')
    if (!session.readContext) throw new Error('The active runtime does not support context export.')
    return { runtime: session.readContext(), preamblePending: !this.primed }
  }

  previewSystemPreamble(text: string): string {
    if (this.primed) return text
    return prependAgentPreamble(text, this.systemPrompt())
  }
  ...
  private withSystemPreamble(message: AgentRuntimePrompt): AgentRuntimePrompt {
    ...
    const text = this.previewSystemPreamble(message.text)
    this.primed = true
    return { ...message, text }
  }
