...
export class PiRuntimeAdapter implements AgentRuntimeAdapter {
  ...
  async createSession(options: AgentRuntimeSessionOptions): Promise<AgentRuntimeSession> {
    ...
    const { session } = await pi.createAgentSession({
      ...
      // Belt and braces with PI_CODING_AGENT_DIR (set at boot): the env var covers pi's frozen
      // TOOLS_DIR and every internal getAgentDir() call, this option covers the resource loader
      // pi builds for THIS session. Without either, pi reads the user's own ~/.pi/agent — whose
      // AGENTS.md lands in our system prompt uncompressed.
...
export class PiRuntimeSession implements AgentRuntimeSession {
  ...
  readContext(): AgentRuntimeContextSnapshot {
    const { systemPrompt, messages } = this.session
    if (typeof systemPrompt !== 'string' || !Array.isArray(messages)) {
      throw new Error('The active pi runtime does not expose readable model context.')
    }
    return snapshotRuntimeContext(systemPrompt, messages)
  }
