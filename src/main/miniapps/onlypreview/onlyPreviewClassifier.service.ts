...
// 取值是 **shiki 的语言 id**,不是「大致像那个语言就行」。
// 2026-09-08 起真正上色的是 shiki 的 TextMate 语法(`onlyPreviewHighlighter.service.ts`),
// 所以一个近似的 id 会把整类语法的颜色丢掉,而不是只差一点:
//   · `.tsx` 曾归 `typescript` —— JSX 那一整层语法在 TS 语法里不存在,整段无色
//   · `.jsx` 曾归 `javascript` —— 同上
//   · `.vue` 曾归 `html` —— `<script setup>` 里的 TS 当纯文本
//   · `.toml` 曾归 `ini` —— ini 语法认不出 TOML 的表头与数组
// 新增或修改一行时,新 id 必须同时在 `onlyPreviewHighlighter.service.ts` 的
// `LANGUAGE_LOADERS` 里有加载器,否则那个语言静默退回不上色。守卫钉住这一条。
