...
import { openOnlyPreviewInDefaultApp } from './onlyPreviewDefaultApp.service';
import type { OnlyPreviewDeleteEntry } from '@shared/onlypreview/onlyPreviewDeleteSelection.shared';
import { xpcMain } from 'electron-xpc/main';
import { ONLY_PREVIEW_BOOKMARK_ADD_EVENT } from '@shared/onlypreview/onlyPreviewBookmarks.type';
...
export class OnlyPreviewProjectNativeActionService {
  async showFileContextMenu(
    ...
    template.push({
      id: 'onlypreview-add-bookmark',
      label: labels.addBookmark,
      enabled: menuSelection.length <= 1 && Boolean(item.relativePath),
      click: () => {
        try {
          this.requireCurrentItem(authority);
          xpcMain.broadcast(ONLY_PREVIEW_BOOKMARK_ADD_EVENT, {
            hostId: authority.host.hostId,
            workspaceId: authority.workspaceId,
            relativePath: item.relativePath
          });
        } catch {
          // The menu belongs to the Project that opened it, not a replacement Project.
        }
      }
    });
  ...
  async openExternally(request: ProjectItemRequestInput): Promise<void> {
    ...
    this.requireCurrentItem(authority);
    await openOnlyPreviewInDefaultApp(file.canonicalPath, () => this.requireCurrentItem(authority));
  }
