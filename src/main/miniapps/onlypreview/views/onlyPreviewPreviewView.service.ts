...
import { installOnlyPreviewSessionProtocol } from '@main/miniapps/onlypreview/onlyPreviewProtocol.service';
...
export interface OnlyPreviewPreviewRegionRuntime {
   ...
   */
  isHostLive: () => boolean;
  // The composite's own parent owns layer geometry and tab visibility. Keyboard focus is global:
  // sibling Chat and Chromium's inner PDF WebContents are not among this container's children.
  container: View;
...
export const presentationAllowsRendererError = (
  ...
  switch (adapterId) {
    ...
    case 'chromium-pdf':
      return false;
    // A directory has no descriptor, so there is no renderer error it could legitimately be
    // reporting — grouped with the two Chromium adapters for the same reason they return false.
    // Reached only once something actually constructs `adapterId: 'directory'`; until then this
    // case exists to keep the exhaustiveness check below honest, which is what caught its absence.
    case 'directory':
      return false;
  }
...
export class OnlyPreviewPreviewViewService {
  ...
  attachActiveView(): void {
    ...
    const newlyAttached = this.attachedView !== view;
    this.attachedView = view;
    onlyPreviewViewLayerService.show('main', 'preview', view);
    view.setBounds({ ...this.contentBounds });
    if (newlyAttached) this.ensureFocusedView(runtime, view);
  }

  /**
   * Recover otherwise missing keyboard focus after attaching a new/replacement preview.
   *
   ...
   *
   * Geometry-only updates never claim focus. Another focused WebContents anywhere in the app wins,
   * including sibling Chat, Project/Search and the PDF viewer's inner WebContents.
   */
  private ensureFocusedView(runtime: OnlyPreviewPreviewRegionRuntime, view: WebContentsView): void {
    try {
      ...
      // A host or view torn down during recovery must not change focus.
    }
  }
