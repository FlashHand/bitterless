...
export class SkillGeneratorService {
  ...
  private async askCodexForDraft(
    ...
    const prompt = [
      'You are Bitterless Maestro. Convert this browser capture into a reusable business skill card.',
      '',
  ...
  private async askCodexForSplitPlan(
    ...
    const startedAt = Date.now()
    const prompt = [
      'You are Bitterless Maestro. This is ONE browser capture that may contain SEVERAL distinct,',
  ...
  private async askCodexToRefine(recipe: SkillRecipe, guidance: string): Promise<CodexSkillDraft> {
    ...
    const prompt = [
      'You are Bitterless Maestro. Improve this EXISTING skill\'s metadata. Keep the same recorded workflow;',
